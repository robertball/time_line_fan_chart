...
def key_press(event):
    global parentViewMode, generationViewMode, individualViewMode, tooltipViewMode
    if event.key == 'q':
        exit()
    if event.key == '1':
        if parentViewMode:
            ...
            viewMode.set_text('(3) Individual View Mode')
        else:
            parentViewMode = True
            generationViewMode = False
            individualViewMode = False
            viewMode.set_text('(1) Parents View Mode')
        ...
        if generationViewMode:
            ...
            individualViewMode = False
            viewMode.set_text('(2) Generation View Mode')
            # viewMode.set_visible(True)
    elif event.key == '3':
        parentViewMode = False
        generationViewMode = False
...
def hover(event):
    if event.inaxes == ax:
        ...
        for patch in reversed(patches):
            _gen = patch.bob['generation']
            _ind = patch.bob['individualNumber']
            _isApproximated = patch.bob['isApproximated']
            cont, ind = patch.contains(event)
            if cont:
                _personID = patch.bob['personID']
                ...
                if individualViewMode:
                    ...
                    patch.set_facecolor(generationToColorDict[_gen])
                    if _isApproximated:
                        patch.set_edgecolor('red')  #663399')  
                        #  set_linewidth(5)
                    else:
        ...
        if foundIt:
            if individualViewMode:
                ...
                for patch in reversed(patches):
                    ...
                    cont, ind = patch.contains(event)
                    if not cont:
                        if _gen in descendants and descendants[_gen] == patch.bob['individualNumber']:
...
def createCustomWedgeWithEndShape(_personID, widthAndHeight1, widthAndHeight2, _generation=0, individualNumber=0, _angle=90,
    ...
    pp.bob = {'generation': _generation, 'individualNumber': individualNumber, 'isApproximated': birthYearApproximated,
              'personID': _personID}
    return pp, vertices1
...
generationToYears = {
    0: [(2018, 2025)],  # 1 person (Heidi)
    1: [(1978, 2025), (1980, 2025)],  # 2 people (Bob and Laura)
    2: [(1950, 2025), (1951, 2025), (1948, 2025), (1949, 2025)],  # 4 people
    ...
    5: [(1866, 1951), (1874, 1959), (1876, 1945), (1884, 1947), (1853, 1908), (1859, 1933), (1849, 1904), (1853, 1921),
        ...
        (1864, 1940)],
    # (1837, 1915), (1836, 1921), (1834, 1919), (1838, 1906), (1865, 1952), (1867, 1948), (1864, 1934), (1864, 1940)],
    #32 people
}
...
annot = ax.annotate("bob", xy=(0, 0), xytext=(20, 20), textcoords="offset points",
...
viewMode = ax.annotate("(3) Individual View Mode", xy=(-180, 180), bbox=dict(boxstyle="round", fc="w"))
viewMode.set_visible(True)
...
for generation in range(numberOfGenerations - 1, -1, -1):
    ...
    for i, person in enumerate(people):
        ...
        if isApproximated:
            custom_path.set_edgecolor('red')  # 663399')
        ...
        if personID == 1:
            ax.text(_center[0], _center[1], str(personID), fontsize=fs, zorder=15, ha='center', va='center',
                    transform=t2)
        else:
            ax.text(_center[0] - 5, _center[1], str(personID), fontsize=fs, zorder=15, ha='center', va='center',
                    transform=t2)
        personID -= 1
